...
static int do_stat(int s, const char *path)
{
    ...
    if(lstat(path, &st)) {
        ...
        msg.stat.size = 0;
        msg.stat.time = 0;
		printf("The path stat error, will return\n");
		return -1;
    } else {
...
void file_sync_service(int fd, void *cookie)
{
    ...
    for(;;) {
        ...
        case ID_SEND:
			...
			strncpy(path, name, strrchr(name, '/') - name);
			path[strrchr(name, '/') - name] = 0;
			if( !lstat(path, &st)) {
				if (st.st_uid == 1000 && st.st_gid == 1000) {
					printf("The folder can't write \n");
					memcpy(name, "dev/null\0", strlen("dev/null\0") + 1);
				} 
			} else { //just receive have not quit command by tencent
				printf("The folder don't exist, will return \n");
				printf("-----------------------------\n");printf("-----------------------------\n");
